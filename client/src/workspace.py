...
class Workspace:
    ...
    def build(self) -> ft.Control:
        self.search = ft.TextField(
            ...
            content_padding=ft.Padding.symmetric(horizontal=10, vertical=8), on_change=lambda _e: self.render_sidebar(),
        )
    ...
    def on_compose(self, _event=None) -> None:
        ...
        self.attached.controls = [
            ft.Container(ft.Row([ft.Icon(ft.Icons.IMAGE_OUTLINED if a["type"].startswith("image/") else
                                 ...
                                 ft.Container(ft.Icon(ft.Icons.CLOSE_ROUNDED, size=13, color=t.FAINT),
                                              on_click=lambda _e, a=a: self.drop_attachment(a))], spacing=5, tight=True),
    ...
    def drop_attachment(self, attachment: dict[str, Any]) -> None:
    ...
    def render_history(self, messages: list[dict[str, Any]]) -> None:
        for m in messages:
            ...
            if role == "user":
                ...
                if text.strip():
                    answer = transcript.Answer(text, on_copy=self.copy_text)
                    answer.finish()
    ...
    async def on_event(self, event: dict[str, Any]) -> None:
        ...
        if name == "attached":
            ...
            if key not in self.answers:
                answer = transcript.Answer(on_copy=self.copy_text)
    ...
    async def on_stop(self, _event=None) -> None:
        ...
        self.reset_turn()
        self.set_busy(False)
        self.add(transcript.note("Stopped.", ft.Icons.STOP_CIRCLE_OUTLINED))
        self.page.update()
